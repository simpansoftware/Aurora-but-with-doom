...
mount -n -o move $(findmnt /tmp/oldroot/tmp/aurora -no SOURCE) /tmp/aurora || echo "Failed to move mountpoint" && sleep 1
mount -n -o move $(findmnt /tmp/oldroot/tmp/aurora/shimroot -no SOURCE) /tmp/shimroot || echo "Failed to move mountpoint" && sleep 1
