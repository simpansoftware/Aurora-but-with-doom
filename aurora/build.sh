...
# You know what else is MASSIVE? The lower tape fade meme.
...
[ -z "$IMAGE" ] && fail "Specify a SH1MMER Legacy image (Feb 2024+): build.sh board.bin"
...
check_raw_shim() {
    ...
    chmod +x "$CGPT"
    "$CGPT" find -l SH1MMER "$LOOPDEV" &>/dev/null || fail "Use a SH1MMER Legacy image!"
